...
평소에는 얇은 알약 모양으로 숨어 있다가, 마우스를 가져가면 메모들이 펼쳐지고, 메모 위에 올리면 내용이 보이며, 클릭하면 바로 편집됨.

## 주요 기능

- 우측 가장자리 도킹: 휴면(알약) -> 팬아웃(탭 목록) -> 확장(본문) 3단계 애니메이션
